...
// `"irreversible"`。
//
// **202 起这个字段只影响显示，不影响 undo**（决策 199 §七/§八）：这里翻出来的
// 名字是 `web:mcp-…`，Rust 侧对 `web:` 一律挡 undo——执行体在这个页面里，还原
// 函数交不回来，声明 `"pure"` 也一样。所以上面那条「默认落保守边」不再是这一层
// 独自扛的安全闸；保留翻译是因为那个自我描述有信息量（工具卡片会连同「本仓不
// 代为补偿」一起显示，见 `../render/tool.ts`）。
