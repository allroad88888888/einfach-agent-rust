...
/** `root/a1/a2` → `a1/a2`——去掉 root 那一段，剩下的路径就是「这是哪个子
 * agent」（跟 `agent-cli` 的 `print::events` 同一条判据，029 判断 20）：
 * `root/a1` 和 `root/a2` 的路径不同,标签自然不同,并行的多个子 agent 因此
 * 分得开,不需要一个额外的「第几个子 agent」计数器。 */
function shortAgentLabel(agent: AgentId): string {
