...
fn a_depth_three_chain_completes_and_the_depth_four_attempt_is_refused() {
    ...
    let server = RoutedServer::start(vec![
        Route { needle: "call_root", delay: Default::default(), status: 200, lines: sse_text("root done, chain complete") },
        Route { needle: "call_l1", delay: Default::default(), status: 200, lines: sse_text("L1 done") },
        Route { needle: "call_l2", delay: Default::default(), status: 200, lines: sse_text("L2 done") },
        // L3 的第二跳：收到 depth 4 被拒的 is_error 之后，自己照常收尾。
        Route { needle: "call_l3_attempt", delay: Default::default(), status: 200, lines: sse_text("L3 done despite the refusal") },
        Route {
            needle: "L1MARK",
            delay: Default::default(),
            status: 200,
            lines: sse_tool_call("call_l1", &spawn_wire, r#"{"task":"L2MARK go two levels down"}"#),
        },
        Route {
            needle: "L2MARK",
            delay: Default::default(),
            status: 200,
            lines: sse_tool_call("call_l2", &spawn_wire, r#"{"task":"L3MARK go three levels down"}"#),
        },
        ...
        Route {
            ...
            delay: Default::default(),
            status: 200,
            lines: sse_tool_call("call_l3_attempt", &spawn_wire, r#"{"task":"L4MARK should never be reached"}"#),
        },
        Route {
            needle: "startchain",
            delay: Default::default(),
            status: 200,
            lines: sse_tool_call("call_root", &spawn_wire, r#"{"task":"L1MARK go one level down"}"#),
        },
    ]);
    ...
    expected.sort();
    assert_eq!(live, expected, "该恰好四个 agent：root/L1/L2/L3，没有 depth 4 的第五个");
    assert_eq!(l3.depth(), 3, "L3 该落在 depth 3——AgentLimits::default() 的上限");
    ...
    let l3_tool_results: Vec<_> = l3_messages
        ...
        .collect();
    assert_eq!(l3_tool_results.len(), 1, "L3 只发起过一次（被拒的）spawn: {l3_messages:#?}");
    assert!(l3_tool_results[0].1, "depth 4 超限该是 is_error: {l3_tool_results:#?}");
    ...
    let l3_final_text: Vec<_> = l3_messages
        ...
        .collect();
    assert!(l3_final_text.iter().any(|t| t.contains("despite the refusal")), "L3 该在拒绝之后照常给出结论: {l3_final_text:#?}");
    ...
    let root_final_text: Vec<_> = session
        ...
        .collect();
    assert!(root_final_text.iter().any(|t| t.contains("chain complete")), "root 该收到链条汇总: {root_final_text:#?}");

    // --- turn_id 全树一致：同覆盖点 1 的证据手法。 ---
    assert_eq!(session.turn_id(), 1, "全程没调过 begin_turn，turn_id 该恒为 1");
    let history_len_before = session.history_len();
    match session.undo_turn() {
        UndoReport::Applied { entries, turn_id } => {
            assert_eq!(turn_id, 1);
            assert_eq!(entries, history_len_before, "四层子树该在同一个 turn 里被一次性吞光");
        }
