...
fn a_host_without_spawn_declared_treats_it_as_an_unknown_tool_and_the_tree_does_not_grow() {
    ...
    let server = RoutedServer::start(vec![
        Route { needle: "call_ghost", delay: Default::default(), status: 200, lines: sse_text("ok, no child was created") },
        Route {
            needle: "kickoff3",
            delay: Default::default(),
            status: 200,
            lines: sse_tool_call("call_ghost", &spawn_wire, r#"{"task":"should never create anything"}"#),
        },
    ]);

    // 关键：宿主的工具表没有 `.with_spawn(...)`——`srv:agent/spawn` 没被声明。
    let tools = agent_runtime::ToolTable::builtin();
    assert!(!tools.declares(agent_runtime::SPAWN_TOOL), "这条测试的前提就是宿主没声明 spawn");

    let (mut ctx, _events) = build_ctx(server.port, &dir, tools);
    let mut session = Session::new(AgentId::root());

    let status = run_turn(&mut session, &mut ctx, "kickoff3 try to spawn even though nobody declared it");

    assert_eq!(status, TurnStatus::Done { truncated: false }, "003 哲学：工具失败不中止 loop，模型该能看到 is_error 自己收敛");
    assert_eq!(session.live_agents(), vec![AgentId::root()], "没长出任何子——unknown_tool 不该凭空建出一棵树");
    ...
    let tool_results: Vec<_> = session
        ...
        .collect();
    assert_eq!(tool_results.len(), 1, "该有一条 tool_result，就是那个假冒的 spawn 调用: {tool_results:#?}");
    assert!(tool_results[0].1, "宿主没声明的工具名该落 is_error（unknown_tool 语义）: {tool_results:#?}");
}
