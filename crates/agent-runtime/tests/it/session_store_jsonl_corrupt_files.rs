...
fn valid_entry_line(seq: u64) -> String {
    let entry = Entry { seq, meta: 1u32, changes: vec![Change { key: "a".to_string(), prev: Val(seq as i64), next: Val(seq as i64 + 1) }] };
    // 跟 `Jsonl` 内部写的格式一样：`{"kind":"entry",...}`——直接拼一行，不经过
    // `agent_runtime` 的私有 `Record` 类型（那是 crate 内部实现，测试只依赖它的
    // 外部可观察行为：这一行 `load()` 得回来）。
    format!(r#"{{"kind":"entry","seq":{},"meta":{},"changes":[{{"key":"{}","prev":{},"next":{}}}]}}"#,
        entry.seq, entry.meta, entry.changes[0].key, entry.changes[0].prev.0, entry.changes[0].next.0)
}
...
fn a_truncated_last_line_is_dropped_with_a_warning_and_the_rest_still_loads() {
    ...
    let loaded = backend.load().loaded().expect("前面那一整条合法记录应该能载入");

    assert_eq!(loaded.entries.len(), 1);
    assert_eq!(loaded.entries[0].seq, 0);
    let seen = errors.lock().unwrap().clone();
    assert_eq!(seen.len(), 1);
    assert!(matches!(seen[0], SessionStoreError::TruncatedTail { line: 2 }));
}
...
fn a_corrupt_middle_line_rejects_the_whole_load_even_though_more_valid_lines_follow() {
    ...
    let reason = match outcome {
        ...
        other => panic!("中部损坏必须整份拒绝（Refused），不能只加载前半段，也不能悄悄当成 Absent：{}", other.is_absent()),
    };
    assert!(reason.contains('2'), "拒绝理由该带上具体行号：{reason}");

    let seen = errors.lock().unwrap().clone();
    assert_eq!(seen.len(), 1);
    assert!(matches!(seen[0], SessionStoreError::CorruptLine { line: 2 }));
}
...
fn the_error_never_carries_the_offending_lines_content() {
    ...
    let content = format!("{{\"kind\":\"entry\",\"seq\":0,\"meta\":1,\"changes\":[{{\"key\":\"{secret}\"\n"); // 断在一半
    write_raw(&path, &content);
    ...
    assert!(!rendered.contains(secret), "错误文本泄漏了行内容：{rendered}");
}
