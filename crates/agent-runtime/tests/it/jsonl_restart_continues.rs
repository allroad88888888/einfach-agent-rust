...
fn a_session_written_through_jsonl_survives_dropping_and_reopening_the_backend() {
    ...
    let backend = agent_runtime::open_backend(Some(session_path.clone()), |e| panic!("不该有加载错误：{e}"));
    let mut recovered = agent_runtime::recover(backend.as_ref(), AgentId::root(), agent_core::DEFAULT_HISTORY_CAP, &mut |k| {
        panic!("不该有不认识的键：{k:?}")
    })
    .unwrap()
    ...
    assert_eq!(status, TurnStatus::Done { truncated: false });
    assert_eq!(recovered.messages().len(), 4, "撤回后的 2 条 + 新一轮的 2 条");
}

fn build_ctx_with_jsonl(port: u16, root: &std::path::Path, session_path: &std::path::Path) -> agent_runtime::RunnerCtx {
    use std::sync::Arc;
    ...
    RunnerCtx::new(
        ...
        agent_runtime::open_backend(Some(session_path.to_path_buf()), |e| panic!("不该有会话文件错误：{e}")),
        Box::new(|_ev| {}),
    )
}
