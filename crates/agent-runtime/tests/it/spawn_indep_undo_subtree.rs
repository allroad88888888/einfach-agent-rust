...
fn undo_after_a_spawn_turn_leaves_no_trace_in_the_next_real_request() {
    ...
    let server = RoutedServer::start(vec![
        Route {
            ...
            status: 200,
            lines: sse_text("root summary SECRET_DELTA"),
        },
        Route {
            needle: "CHILDTASK",
            delay: Default::default(),
            status: 200,
            lines: sse_text("child result SECRET_GAMMA"),
        },
        Route {
            needle: "firstturn",
            delay: Default::default(),
            ...
            lines: sse_tool_call(
                "call_child",
                &spawn_wire,
                r#"{"task":"CHILDTASK handle SECRET_BETA"}"#,
            ),
        },
        Route {
            needle: "secondturn",
            delay: Default::default(),
            status: 200,
            lines: sse_text("second turn answer, nothing to do with the first"),
        },
    ]);
    ...
    let second_request = server
        .call("secondturn")
        .expect("second turn must have gone out for real");
    for trace in [
        "firstturn",
        "SECRET_ALPHA",
        "CHILDTASK",
        ...
        assert!(
            !second_request.body.contains(trace),
            "第二轮请求体不该含第一轮的痕迹 {trace:?}: {}",
            second_request.body
        );
    }
