...
pub fn read_request_body(stream: &mut TcpStream) -> String {
    let mut reader = BufReader::new(stream.try_clone().unwrap());
    let mut content_length = 0usize;
    loop {
        let mut line = String::new();
        if reader.read_line(&mut line).unwrap_or(0) == 0 {
            return String::new(); // 对端提前断开
        }
        if line == "\r\n" || line.is_empty() {
            break;
        }
...
pub enum ScriptedResponse {
    ...
    Sse(Vec<&'static str>),
    /// Write an initial SSE group, pause, then mark and write the terminal group.
    SseSplit {
        ...
        for resp in responses {
            ...
            match resp {
                ...
                ScriptedResponse::SseSplit {
                    ...
                    after,
                    after_started,
                } => write_split(&mut stream, before, after, &after_started),
...
/// Scripted server variant that records each JSON request body for prompt-boundary assertions.
pub fn spawn_recording_server(responses: Vec<ScriptedResponse>) -> (u16, Arc<Mutex<Vec<String>>>) {
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let port = listener.local_addr().unwrap().port();
    ...
    std::thread::spawn(move || {
        for resp in responses {
            ...
            sink.lock().unwrap().push(read_request_body(&mut stream));
            write_sse_headers(&mut stream);
            match resp {
                ScriptedResponse::Sse(lines) => {
                    write_lines(&mut stream, lines);
...
fn write_lines(stream: &mut TcpStream, lines: Vec<&'static str>) {
    for line in lines {
...
fn write_split(
    ...
    before: Vec<&'static str>,
    after: Vec<&'static str>,
    after_started: &AtomicBool,
) {
    write_lines(stream, before);
    std::thread::sleep(Duration::from_millis(200));
    after_started.store(true, Ordering::Release);
    write_lines(stream, after);
}
...
pub fn build_ctx_with_observer(
    port: u16,
    root: &std::path::Path,
    tools: ToolTable,
    store: Option<PathBuf>,
    ...
    let ctx = RunnerCtx::new(
        ...
        Vec::new(),
        session_config,
        agent_runtime::open_backend(store, |_| {}),
        Box::new(move |ev| {
