...
fn two_children_run_in_parallel_and_the_parent_waits_for_both() {
    ...
    assert!(b.end < a.end, "脚本让 B 快 A 慢，B 该先回：{:?} vs {:?}", b.end, a.end);
    assert!(
    ...
    assert_eq!(root.len(), 4, "user → assistant(2×ToolUse) → assistant(2×ToolResult) → assistant(文本)：{root:#?}");
    ...
    let results: Vec<(&str, bool)> = root[2]
        ...
        .collect();
    assert_eq!(results, vec![("结果A：甲是 1。", false), ("结果B：乙是 2。", false)]);

    let child_a = AgentId::new("root/a1");
    let child_b = AgentId::new("root/a2");
    let a_msgs = session.messages_of(&child_a);
    assert_eq!(a_msgs.len(), 2, "子 agent：task 那条 user + 它自己的回复：{a_msgs:#?}");
    ...
    // —— 4. turn_id 全树一致 ————————————————————————
    let turns: Vec<u64> = session.history().entries().map(|e| e.meta.turn_id).collect();
    assert!(!turns.is_empty());
    assert!(turns.iter().all(|t| *t == session.turn_id()), "整棵树同一个 turn_id：{turns:?}");
    let agents: Vec<String> = session
        .history()
        .entries()
        .flat_map(|e| e.changes.iter().map(|c| c.key.agent().as_str().to_string()))
        .collect();
    assert!(agents.iter().any(|a| a == "root/a1"), "子 agent 的写入落在同一条日志上：{agents:?}");

    // —— 事件归属：子 agent 的事件带着自己的 id ————————————————
    let events = events.borrow();
    assert!(
        events.iter().any(|e| e.agent == child_a && matches!(e.event, RunnerEvent::TurnGuard { .. })),
...
fn undoing_the_turn_takes_the_whole_subtree_and_the_next_prompt_has_no_trace_of_it() {
    ...
    let mut session = Session::new(AgentId::root());
    assert_eq!(run_turn(&mut session, &mut ctx, "分头去查甲和乙"), TurnStatus::Done { truncated: false });

    let child_a = AgentId::new("root/a1");
    assert!(!session.messages_of(&child_a).is_empty());

    let report = session.undo_turn();
    assert!(matches!(report, agent_core::UndoReport::Applied { .. }), "spawn 是 Reversible，不该被屏障挡住：{report:?}");
    assert!(session.messages().is_empty(), "root 的消息全退了");
    assert!(session.messages_of(&child_a).is_empty(), "子 agent 的消息也全退了");
    assert!(!session.is_live(&child_a), "spawn 被撤销之后子 agent 不在活名单上（028 的裁决）");

    assert_eq!(run_turn(&mut session, &mut ctx, "再问一次"), TurnStatus::Done { truncated: false });
    let reask = server.call("再问一次").expect("重问那一跳该被服务过");
    for trace in ["任务A", "任务B", "结果A", "结果B", "srv_3Aagent_2Fspawn\",\"arguments"] {
        assert!(!reask.body.contains(trace), "撤销之后的 prompt 不该再有子树痕迹「{trace}」：{}", reask.body);
    }
}
