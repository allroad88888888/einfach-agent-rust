...
fn a_web_name_the_table_never_declared_gets_an_is_error_result_instead_of_a_waiting_slot() {
    ...
    let port = spawn_scripted_server(vec![
        sse_tool_call("call_ghost", &wire_name::to_wire(ghost), r#"{\"any\": \"thing\"}"#),
    ...
    assert_eq!(status, TurnStatus::Done { truncated: false }, "该跑完第二跳收敛，而不是停在 ToolsPending");
    // 2. **没进等待槽**——060 的核心断言。
    assert_eq!(ctx.pending_remote_tool_count(), 0, "编出来的 web: 名字不该占一个永远等不到回传的槽");
    ...
    assert!(results[0].1, "未声明的工具名该落 is_error（unknown_tool 语义）: {results:#?}");
    assert!(results[0].0.contains("unknown_tool"), "错误里该说清是「不认识这个工具」: {results:#?}");
}
...
fn a_declared_web_tool_still_parks_in_the_waiting_slot() {
    ...
    assert_eq!(status, TurnStatus::ToolsPending, "声明过的远端工具该等宿主回传");
    assert_eq!(ctx.pending_remote_tool_count(), 1, "它该正好占一个等待槽");
    assert!(ctx.next_remote_deadline().is_some(), "060：占了槽就该有截止线");
