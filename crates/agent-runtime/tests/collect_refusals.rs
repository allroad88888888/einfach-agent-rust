...
fn collecting_twice_an_unknown_id_or_a_non_descendant_all_come_back_as_errors() {
    ...
    let server = RoutedServer::start(vec![
        Route { needle: "call_self", delay: Duration::ZERO, status: 200, lines: sse_text("好吧，剩下的我自己答") },
        Route {
            needle: "call_ghost",
            delay: Duration::ZERO,
            status: 200,
        ...
        Route { needle: "TASKR", delay: Duration::ZERO, status: 200, lines: sse_text("ANSWERR 领一次就好") },
        Route {
            needle: "kickoff",
            delay: Duration::ZERO,
            status: 200,
    ...
    // 三次拒绝一次都没有打断这一轮。
    assert_eq!(status, TurnStatus::Done { truncated: false }, "拒绝是给模型看的文本，不是这一轮的结局");

    let results = tool_results(&session, &AgentId::root());
    assert_eq!(results.len(), 5, "spawn + 一次成功的 collect + 三次拒绝：{results:#?}");
    ...
    assert!(results[2].1.contains("root/a1"), "拒绝文本该点名是哪个：{}", results[2].1);
    assert!(results[2].1.contains("只能领一次"), "该说清为什么：{}", results[2].1);
    ...
    assert!(results[4].2, "领自己该被拒（红线 10 一条例外都没有）：{results:#?}");
    assert!(results[4].1.contains("后代"), "拒绝文本该说清规则：{}", results[4].1);
