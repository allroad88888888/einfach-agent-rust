//! Stable SHA-256 used by the bounded remote-tool receipt ledger.
//!
//! Receipts must compare a replayed payload without retaining the payload itself.  Keeping the
//! digest implementation here avoids tying that runtime-only registry to a provider or transport.
