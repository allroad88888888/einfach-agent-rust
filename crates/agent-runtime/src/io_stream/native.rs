...
pub(crate) fn open(
    ...
    thread::spawn(move || blocking_source(tx, binding, body, cancel));
    rx
}

/// 工作线程本体：把现成的请求体发出去 → 逐行往回送。**所有阻塞都发生在这里。**
///
/// 请求体是入参：编码在 `provider_call::start` 里就做完了（见 [`super`] 模块
/// 文档「请求体是入参」），这条线程上没有任何「准备」步骤。
///
