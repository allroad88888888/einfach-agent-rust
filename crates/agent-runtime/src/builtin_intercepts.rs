//! 147：既有四条工具截获（spawn/collect/status/skill-read）迁进
//! [`crate::intercept_registry`] 的表。
//!
//! # 这是「哪些内置工具用这张表」，不是「这张表怎么工作」
//!
//! 机制本身（`InterceptArgs`/`InterceptFn`/自借用陷阱/撞名判据）住
//! `intercept_registry.rs`——那是通用的装配期正门，不认识任何具体工具。这个
//! 文件反过来：只认识四个具体工具，不管表怎么实现。拆开是「一个文件一件事」
//! （红线 9）：注册表的自借用/撞名/公开签名已经够那个文件讲一整篇，再塞进
//! 「这四个工具具体转发给谁」会让两件不同颗粒度的事挤在一起。
//!
//! # 内部层直调，不走 `SessionToolFn`
//!
//! 这四条截获要碰的东西够不着公开层的收窄签名（`SessionToolFn` 只留
...
//! 这四条能拿到完整的 `InterceptArgs`，包一层反而会让 `ToolExecuting` 发两次
//! ——147 的验收就是揪住这个不许发生。
//!
//! # 注册点：`RunnerCtx::new` 的构造链
//!
//! [`register_builtin_intercepts`] 在那之后立刻调用，按 `declares()` 判断要不要
//! 注册——跟各自 `with_*`（`with_spawn`/`with_collect`/`with_status`/
//! `with_skills`）让 `declares()` 为真的时机同一条件，声明与执行路径因此天然
