...
//!
//! `web:` 工具走的是 M10 的远端等待槽：`run_turn` 会带着一个非终态的
//! `ToolsPending` 返回，等宿主调 `resolve_remote_tool`。server 形态下那一步隔着
//! 一次 HTTP 往返，所以要把控制权交还给客户端；浏览器形态下**宿主就是我们
//! 自己**，两端同进程，没有任何理由把一轮对话切成两次 JS 调用——页面说一句话
//! 就该等到一个答案。所以这个函数把「执行 + 回传」就地做掉，页面看到的仍然是
//! 一个 Promise 一轮对话。
//!
//! 循环为什么不会空转：每一圈**消费掉恰好一个**等待槽（`resolve_remote_tool`
...
use agent_runtime::{RunnerCtx, resolve_remote_tool, run_turn};
...
/// 跑一整轮。
pub(crate) async fn run(session: &mut Session, ctx: &mut RunnerCtx, text: &str) -> Outcome {
    if session.status().is_terminal() {
        session.begin_turn();
        agent_runtime::persist::sync(ctx, session);
    }
    // 取消标志的清零在 `run_turn` 内部做（每轮开始各清一次），这里不重复。
    let status = run_turn(session, ctx, text).await;
...
async fn drain_host_tools(
    ...
    loop {
        ...
        match resolve_remote_tool(session, ctx, waiting.agent, waiting.call_id, output).await {
            Ok(next) => status = next,
