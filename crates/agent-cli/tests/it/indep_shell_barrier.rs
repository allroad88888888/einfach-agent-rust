...
fn undo_stops_at_a_shell_exec_barrier_and_undo_force_crosses_it() {
    ...
    let server = FakeServer::start(vec![
        Script::Immediate(sse::tool_call(
            "call_1",
    ...
    assert!(
        after_turn.contains("srv:shell/exec"),
        "该看到工具执行日志：{after_turn}"
    );
    assert_eq!(
    ...
    assert!(
        blocked.contains("srv:shell/exec"),
        "撤销受阻消息该点名工具：{blocked}"
    );
    assert!(
        blocked.contains("call_1"),
        "撤销受阻消息该带 call_id：{blocked}"
    );
    assert!(
        blocked.contains("/undo!"),
        "撤销受阻消息该给出明确的越过指引：{blocked}"
    );
    ...
    assert!(
        !cli.stdout_snapshot().contains("没有可重做的了"),
        "不该是无事可做"
    );
    ...
    assert!(
        forced.contains("[已撤销]"),
        "/undo! 之后整轮该被撤销：{forced}"
    );
    assert!(
        forced.contains("srv:shell/exec"),
        "越过提示该点名工具：{forced}"
    );
    ...
    assert_eq!(
        full.matches("srv:shell/exec 完成").count(),
        1,
        "整个会话期间 shell 只该被真正执行一次：{full}"
    );
}
