//! 装配 + 起服务 + Ctrl-C 优雅退出（issue 035）：provider 装配交给
//! `agent_server::bootstrap`（跟 `examples/serve.rs` 共用同一份逻辑，见那个
//! 函数的模块文档），这个文件只管这个 bin 独有的部分——`--sessions-dir`/
//! `--port`/`--config` 这三个 flag 怎么落地成 `bootstrap`/`bind` 的输入、
//! 启动横幅打什么、Ctrl-C 触发时怎么收尾。`main.rs` 只解析参数、调
//! [`run`]，装配细节不出现在那个文件里。

use std::sync::Arc;

use agent_core::{AgentLimits, SystemChunk};
use agent_server::{AgentServer, BoundAgentServer, BootstrapOptions, ServerConfig, ToolTableSpec, bootstrap, default_bind_addr};
...
pub async fn run(cli: Cli) {
    ...
    let assembled = bootstrap(BootstrapOptions {
        ...
        system: vec![SystemChunk { label: Arc::from("base"), text: Arc::from(SYSTEM_PROMPT) }],
    ...
    let port =
        cli.port.or_else(|| std::env::var("AGENT_SERVER_PORT").ok().and_then(|s| s.parse().ok())).unwrap_or(0);
    ...
    let bound = server.bind(addr).await.unwrap_or_else(|e| fail(&format!("绑定 {addr} 失败: {e}")));
...
fn print_banner(bound: &BoundAgentServer, provider_name: &str, model: &str, cli: &Cli) {
    eprintln!("agent-server 监听 http://{}（provider={provider_name} model={model} tools=builtin+shell+spawn，开满档）", bound.local_addr());
    match &cli.sessions_dir {
        Some(dir) => eprintln!("会话目录={}（POST /sessions 不带 session_path 时自动落盘到这里）", dir.display()),
        None => eprintln!("会话目录=（未指定，POST /sessions 不带 session_path 就是内存会话——用 --sessions-dir <dir> 落盘）"),
    }
    eprintln!("Ctrl-C 退出（优雅：所有会话落盘快照之后才退出）。");
}
