...
//!
//! 两处 primitive 状态是**同一个形状**：spawn 的工具子集（`Slot::ToolsAllowed`）、
//! 039 激活的 skill 集（`Slot::SkillsActive`）——都是「一组字符串，排序去重后落成
//! JSON 字符串数组」。抽成一处，是因为红线 11 的「排序去重才逐字节确定」这一步
//! 不能在两个地方各写一遍：写漏一处就是那一处**每轮全价**，而且不报错、功能正常，
