...
//! ## 红线 11：排序 + 稳定字节
//!
//! [`to_value`] **按 id 排序**再落值：客户端给的数组顺序不可靠（两次连接可能不同
//! 序），而它会经 registry 变成索引行的 prompt 字节。`serde_json::Value` 的对象
//! 后端是 `BTreeMap`（根 `Cargo.toml` 显式不开 `preserve_order`），所以同一份声明
//! 两次序列化逐字节相同——`schema` 那个自由 JSON 也一样，这条有断言钉住，不是假设。
//!
//! ## 自带工具的 `reversibility` **不落盘**（如实记一笔）
//!
//! 061 的协议形状里，skill 自带的工具跟顶层工具是同一个类型，所以它也能写
//! `reversibility`。但 `late_tools` 今天**连 `declares()` 都不进**（069 §另记一笔
//! 记的那个可执行性洞），没有任何一处会去查它的可逆性。存一个没有读者的字段只会
//! 给将来一个「它一直是对的」的错觉——真要把 `late_tools` 接上执行，那时该定的是
//! 「激活时它进不进表」这个更大的问题，不是从历史里挖一个当年顺手存下的值。
//! 所以这里只落三个**进 prompt** 的字段，跟 [`crate::ToolSpec`] 一一对应。
...
pub struct HostSkill {
    ...
    pub body: Arc<str>,
    /// 激活时进 `late_tools` 的工具。三个字段就是进 prompt 的那三个；可逆性不落，
    /// 理由见模块文档。
    #[serde(default)]
    pub tools: Vec<ToolSpec>,
}

/// 按 id 排序 → JSON 对象数组值（红线 11）。
pub(crate) fn to_value(mut skills: Vec<HostSkill>) -> AgentValue {
    ...
    let items: Vec<serde_json::Value> = skills
        .into_iter()
        .map(|skill| serde_json::to_value(skill).expect("四个字段全是可序列化的普通类型"))
        .collect();
...
mod tests {
    ...
    fn the_bytes_do_not_depend_on_input_order_anywhere() {
        ...
        let with = |schema: &serde_json::Map<String, serde_json::Value>| {
            let make = |id: &str, tool: &str| HostSkill {
                id: SkillId::new(id),
                description: Arc::from("一行描述"),
                body: Arc::from("正文若干"),
                tools: vec![ToolSpec {
