...
fn epoch_bearing_variants_return_some() {
    ...
    let cases: Vec<Event> = vec![
        ...
        Event::ToolResult { agent: agent.clone(), epoch, call_id: support::call_id(), content: Arc::from("x") },
        Event::ToolFailed { agent: agent.clone(), epoch, call_id: support::call_id(), error: Arc::from("x") },
        Event::Timeout { agent: agent.clone(), epoch, call_id: None },
        Event::Timeout { agent: agent.clone(), epoch, call_id: Some(support::call_id()) },
    ];
...
fn user_input_and_cancel_return_none() {
    let agent = support::agent();
    assert_eq!(
        Event::UserInput { agent: agent.clone(), text: Arc::from("hi") }.epoch(),
        None,
        "UserInput 不带 epoch"
    );
