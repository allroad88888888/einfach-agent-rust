...
fn depth_four_is_refused_without_panicking() {
    ...
    for _ in 0..3 {
        current = session.spawn_child(&current, ChildConfig::default()).expect("depth <= 3 应该被允许");
    }
    assert_eq!(current.depth(), 3);

    let refused = session.spawn_child(&current, ChildConfig::default());
    assert_eq!(refused, Err(SpawnRefused::DepthExceeded { depth: 4, max: 3 }));
    assert!(session.children_of(&current).is_empty(), "被拒的 spawn 不该留下任何痕迹");
}
...
fn the_ninth_sibling_is_refused_without_panicking() {
    ...
    for _ in 0..8 {
        session.spawn_child(&root, ChildConfig::default()).expect("前 8 个该被允许");
    }
    assert_eq!(session.children_of(&root).len(), 8);

    let refused = session.spawn_child(&root, ChildConfig::default());
    assert_eq!(refused, Err(SpawnRefused::TooManyChildren { live: 8, max: 8 }));
...
fn set_agent_limits_does_not_retroactively_kill_existing_children() {
    ...
    for _ in 0..8 {
        session.spawn_child(&root, ChildConfig::default()).expect("先在默认上限内长满");
    }
    assert_eq!(session.children_of(&root).len(), 8);

    session.set_agent_limits(AgentLimits { max_depth: 3, max_children: 2 });
    assert_eq!(session.agent_limits(), AgentLimits { max_depth: 3, max_children: 2 });
    ...
    let refused = session.spawn_child(&root, ChildConfig::default());
    assert_eq!(refused, Err(SpawnRefused::TooManyChildren { live: 8, max: 2 }));
}

#[test]
fn lowering_max_depth_does_not_kill_an_existing_deep_agent() {
    let mut session = new_session();
    let a1 = session.spawn_child(&session.agent().clone(), ChildConfig::default()).expect("depth 1");
    let a2 = session.spawn_child(&a1, ChildConfig::default()).expect("depth 2");
    let a3 = session.spawn_child(&a2, ChildConfig::default()).expect("depth 3");
    assert_eq!(a3.depth(), 3);

    session.set_agent_limits(AgentLimits { max_depth: 1, max_children: 8 });

    assert!(session.is_live(&a3), "已经存在的深度 3 agent 不该被追溯清理");

    let refused = session.spawn_child(&a1, ChildConfig::default());
    assert_eq!(refused, Err(SpawnRefused::DepthExceeded { depth: 2, max: 1 }));
}
