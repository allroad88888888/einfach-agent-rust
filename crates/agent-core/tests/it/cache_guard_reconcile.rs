...
fn matching_prediction_is_silent() {
    ...
    assert_eq!(v, ReconcileVerdict::Match { predicted: 512, actual: 512 });
...
fn shortfall_beyond_threshold_alerts_with_numbers() {
    ...
    assert_eq!(v, ReconcileVerdict::Shortfall { predicted: 5120, actual: 1024, gap: 4096 });
...
fn thirty_percent_boundary_has_a_definite_side() {
    ...
    assert_eq!(
        ...
        ReconcileVerdict::Match { predicted: 1000, actual: 700 }
    );
    assert_eq!(
        reconcile(1000, Some(699), p),
        ReconcileVerdict::Shortfall { predicted: 1000, actual: 699, gap: 301 }
    );
}
...
fn better_than_expected_is_information_not_alarm() {
    ...
    assert_eq!(
        v,
        ReconcileVerdict::BetterThanExpected { predicted: 512, actual: 5120, surplus: 4608 }
    );
...
fn missing_field_and_explicit_zero_take_different_branches() {
    ...
    assert_eq!(zero, ReconcileVerdict::Shortfall { predicted: 512, actual: 0, gap: 512 });
...
fn zero_prediction_is_not_judged() {
    ...
    assert_eq!(reconcile(0, Some(0), p), ReconcileVerdict::NoPrediction { actual: 0 });

    // 无预测 + 这家不报 → 先说失明：没有真实数字，谈不上对账。
    assert_eq!(reconcile(0, None, p), ReconcileVerdict::Blind { predicted: 0 });
}
...
fn tolerance_and_threshold_are_parameters() {
    ...
    let loose = ReconcileParams { tolerance_tokens: 64, ..ReconcileParams::default() };
    assert!(matches!(reconcile(100, Some(101), loose), ReconcileVerdict::Match { .. }));
    assert!(matches!(reconcile(100, Some(50), loose), ReconcileVerdict::Match { .. }));

    // 阈值收紧到 10%：同一份数字就该告警了。
    let strict = ReconcileParams { shortfall_alert_percent: 10, ..ReconcileParams::default() };
    assert!(matches!(reconcile(1000, Some(880), strict), ReconcileVerdict::Shortfall { gap: 120, .. }));
}
...
fn layer2_is_pure() {
    let p = ReconcileParams::default();
    let cases = [(0u32, None), (0, Some(1)), (512, Some(512)), (512, None), (1000, Some(1))];
