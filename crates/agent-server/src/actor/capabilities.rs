...
//!
//! skill 这一路当初（064）还多一条**它独有**的理由：`Slot::SkillsActive` 早就在
//! store 里了。声明不落盘，恢复出来就是一份指向空 registry 的激活集——状态说某个
//! skill 激活着、展开注入却什么都取不到（查不到的 id 静默跳过），而模型的历史里
//! 明明写着它读过那段正文。**悬空引用 + 静默**，正是本仓最怕的形状。141 之后
//! `SkillsActive` 没有任何代码再读它去展开注入了，这段风险描述的是**当初为什么
//! 定这条决策**，不是今天还会发生的事——但 `HostSkills` 该不该 journaled 这条
//! 结论没变：`body`（正文）本身仍然是「只在那一次 HTTP 请求里存在过」的资产，
//! 不落盘就永久没了。
//!
