...
pub(in crate::http) async fn undo(State(state): State<AppState>, Path(id): Path<String>, ApiJson(body): ApiJson<UndoRequest>) -> Result<StatusCode, ApiError> {
    ...
    if matches!(body.granularity, GranularityWire::Step) && body.force {
        return Err(ApiError::bad_request("granularity: \"step\" 不支持 force：Session 没有 undo_step 的越过屏障变体，只有 turn 粒度有 /undo! 那档"));
    }
    let cmd = Command::Undo { granularity: body.granularity.into(), force: body.force };
    state.dispatch(&SessionId::from(id), cmd)?;
    Ok(StatusCode::ACCEPTED)
}

pub(in crate::http) async fn redo(State(state): State<AppState>, Path(id): Path<String>) -> Result<StatusCode, ApiError> {
