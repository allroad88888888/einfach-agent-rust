...
async fn a_valid_declaration_creates_the_session_and_stays_idempotent() {
    ...
    let created = create(addr, json!({ "id": CHAT_ID, "capabilities": valid_capabilities() }));
...
async fn rejected_declarations_never_create_a_session() {
    ...
    let cases: Vec<(&str, Value)> = vec![
        ("srv: 前缀", json!({ "tools": [ { "name": "srv:crm/lookup" } ] })),
        ("mcp: 前缀", json!({ "tools": [ { "name": "mcp:everything/echo" } ] })),
        ("无前缀", json!({ "tools": [ { "name": "crm_lookup" } ] })),
        ("空名", json!({ "tools": [ { "name": "" } ] })),
        ("只有前缀", json!({ "tools": [ { "name": "web:" } ] })),
        ("前缀之后有空格", json!({ "tools": [ { "name": "web:crm lookup" } ] })),
        // 最容易漏的一条：skill 自带的工具走同一条校验。
        ("skill 自带 srv: 工具", json!({ "skills": [ { "id": "crm-flow", "tools": [ { "name": "srv:crm/lookup" } ] } ] })),
        ("顶层工具撞名", json!({ "tools": [ { "name": "web:a/b" }, { "name": "web:a/b" } ] })),
        (
            "skill 工具与顶层撞名",
            json!({ "tools": [ { "name": "web:a/b" } ], "skills": [ { "id": "s1", "tools": [ { "name": "web:a/b" } ] } ] }),
        ),
        ("skill id 撞名", json!({ "skills": [ { "id": "s1" }, { "id": "s1" } ] })),
        ("skill id 字符集", json!({ "skills": [ { "id": "crm/flow" } ] })),
    ...
    for (label, capabilities) in cases {
        ...
        assert!(response.body.contains("\"bad_request\""), "{label}：{}", response.body);
        assert!(sessions.ids().is_empty(), "{label}：不合规的声明不得登记 session：{:?}", sessions.ids());
        let status = http_client::request(addr, "GET", &format!("/sessions/{CHAT_ID}"), None);
        assert_eq!(status.status, 404, "{label}：会话不该存在过：{}", status.body);
    }
}
...
async fn the_rejection_message_says_which_item_and_why() {
    ...
    let response = create(addr, json!({ "capabilities": { "tools": [ { "name": "srv:crm/lookup" } ] } }));
    assert_eq!(response.status, 400, "{}", response.body);
    assert!(response.body.contains("srv:crm/lookup"), "该指出是哪一项：{}", response.body);
    assert!(response.body.contains("capabilities.tools"), "该指出在哪儿声明的：{}", response.body);
    assert!(response.body.contains("web:"), "该说清合法前缀是什么：{}", response.body);

    let response = create(addr, json!({ "capabilities": { "skills": [ { "id": "s1", "tools": [ { "name": "srv:x/y" } ] } ] } }));
    assert!(response.body.contains("skill \\\"s1\\\""), "该指出是哪个 skill 带的：{}", response.body);
...
async fn omitting_or_emptying_capabilities_keeps_the_old_behavior() {
    ...
    assert!(!legacy.body.contains("\"outcome\""), "旧请求的响应形状不变：{}", legacy.body);

    for body in [json!({ "capabilities": {} }), json!({ "capabilities": { "tools": [], "skills": [] } })] {
        let response = create(addr, body.clone());
        assert_eq!(response.status, 201, "{body}：{}", response.body);
    }

    assert_eq!(sessions.ids().len(), 3, "三次请求各建一个会话：{:?}", sessions.ids());
    assert!(sessions.close_all().iter().all(|(_, result)| result.is_ok()));
}
