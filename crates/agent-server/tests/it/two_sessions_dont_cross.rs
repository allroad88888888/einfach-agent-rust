...
async fn two_sessions_run_concurrently_without_crossing_events() {
    ...
    let handle_a = registry.open(support::open_spec("session-a", server_a.endpoint(), None)).unwrap();
    let handle_b = registry.open(support::open_spec("session-b", server_b.endpoint(), None)).unwrap();

    let mut sub_a = handle_a.subscribe();
    let mut sub_b = handle_b.subscribe();

    handle_a.send(agent_server::Command::Input("hi from a".to_string())).unwrap();
    handle_b.send(agent_server::Command::Input("hi from b".to_string())).unwrap();
    ...
    assert!(!text_a.contains("from-B"), "session A 的订阅者不该看到 B 的文本");
    assert!(!text_b.contains("from-A"), "session B 的订阅者不该看到 A 的文本");
