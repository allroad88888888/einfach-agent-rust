...
async fn a_last_event_id_older_than_the_ring_buffer_yields_an_explicit_gap_frame() {
    ...
    let (status, _, mut sse) =
        http_client::connect_sse(server.addr, &format!("/sessions/{id}/events"), Some(1));
    ...
    assert!(
        matches!(envelope.event, SessionEvent::Gap { .. }),
        "缓冲区早就挤掉 id 1 了，第一帧该是 Gap：{envelope:?}"
    );
    assert_eq!(
        envelope.agent,
        AgentId::root(),
        "gap 帧是重连补发算出来的传输层事实，该标 root"
    );
}
