...
async fn a_recovered_session_brings_its_declared_skills_back_without_being_told_again() {
    ...
    assert!(system_text(&after).contains("crm-flow: 处理客户工单的标准流程"), "{}", system_text(&after));
...
async fn undoing_the_activation_takes_the_body_out_of_the_next_round() {
    ...
    assert!(system_text(&activated).contains(CRM_BODY), "激活之后的下一跳该带上正文：{}", system_text(&activated));

    // ── B：再说一句话，正文**仍然在**（激活是会话状态，不是那一跳的临时料）。
    let still = one_turn(&upstream, addr).await;
    assert!(system_text(&still).contains(CRM_BODY), "激活是 journaled 的会话状态，下一轮该还在：{}", system_text(&still));

    // ── C：撤掉两轮（刚才这一轮对话 + 激活那一轮）→ 正文消失，索引行还在。
    undo(addr);
    undo(addr);
    let after_undo = one_turn(&upstream, addr).await;
    assert!(!system_text(&after_undo).contains(CRM_BODY), "undo 越过激活之后正文该消失：{}", system_text(&after_undo));
    assert!(
