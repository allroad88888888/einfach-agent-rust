...
async fn undo_blocked_by_a_shell_barrier_carries_the_tool_name_and_call_id_over_sse() {
    let upstream = FakeServer::start(vec![Script::Immediate(tool_call_reply()), Script::Immediate(support::wire::text_reply("done"))]);
    ...
    let input = http_client::request(server.addr, "POST", &format!("/sessions/{id}/input"), Some("{\"text\":\"跑个命令\"}"));
    ...
    assert_eq!(frame.agent.as_str(), "root", "/undo 是会话级命令，该标 root");
    let SessionEvent::Undo(UndoOutcome::Blocked { entries, tool, call_id, label, .. }) = frame.event.clone() else {
    ...
    assert_eq!(tool.as_deref(), Some("srv:shell/exec"), "该带工具名，不是只有裸的 barrier_seq：{frame:?}");
    assert_eq!(call_id.as_deref(), Some("call_shell_1"), "该带 call_id：{frame:?}");
    assert_eq!(label, "tool_result", "被撞上的这条 entry 是工具结果落地那一条：{frame:?}");
}
