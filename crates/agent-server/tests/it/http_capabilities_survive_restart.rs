...
async fn a_recovered_session_brings_its_declared_tools_back_without_being_told_again() {
    ...
    let created = create(first_addr, json!({ "id": CHAT_ID, "capabilities": declaration() }));
    assert_eq!(created.status, 201, "{}", created.body);
    assert_eq!(support::extract_json_string_field(&created.body, "outcome"), "created");
    ...
    assert_eq!(
        names(&after),
        vec!["srv:fs/read", "srv:fs/list", "desk:clipboard/write", "web:crm/lookup"],
...
async fn declaring_again_on_a_session_with_history_is_refused_with_its_own_error_code() {
    ...
    let (first_addr, first_sessions) = start(persistent_template(upstream.endpoint(), sessions_dir.clone())).await;
    assert_eq!(create(first_addr, json!({ "id": CHAT_ID, "capabilities": declaration() })).status, 201);
    ...
    // ── 又带一次声明（哪怕跟当初一模一样）：拒。
    let refused = create(addr, json!({ "id": CHAT_ID, "capabilities": declaration() }));
    assert_eq!(refused.status, 400, "{}", refused.body);
    assert!(refused.body.contains("\"session_has_history\""), "要有自己的错误码，不能只是通用 bad_request：{}", refused.body);
    assert!(refused.body.contains("GET /sessions/"), "错误文案该告诉调用方怎么判断该不该带声明：{}", refused.body);
    assert!(sessions.ids().is_empty(), "被拒的请求不该把会话 open 起来：{:?}", sessions.ids());

    // ── 同一个会话、名字写错的声明：**另一个**错误码（这才叫「能区分」）。
    let bad_name = create(addr, json!({ "id": CHAT_ID, "capabilities": { "tools": [ { "name": "srv:crm/lookup" } ] } }));
    assert_eq!(bad_name.status, 400, "{}", bad_name.body);
    assert!(bad_name.body.contains("\"session_has_history\""), "有历史这条排在名字校验前面——这次的声明再正确也不会被采纳，先说这个更有用：{}", bad_name.body);
...
async fn the_status_endpoint_tells_a_chatid_with_history_from_one_without() {
    ...
    assert_eq!(create(addr, json!({ "id": CHAT_ID, "capabilities": declaration() })).status, 201);
    ...
    assert_eq!(dormant.status, 200, "关掉的会话磁盘上还有历史，答 404 就等于骗客户端「这是新会话，带上声明吧」：{}", dormant.body);
...
async fn undoing_past_the_declaration_takes_the_injected_tools_out_of_the_table() {
    ...
    let (a_addr, a_sessions) = start(persistent_template(upstream.endpoint(), sessions_dir.clone())).await;
    assert_eq!(create(a_addr, json!({ "id": CHAT_ID, "capabilities": declaration() })).status, 201);
    ...
    let (b_addr, b_sessions) = start(persistent_template(upstream.endpoint(), sessions_dir.clone())).await;
...
fn undo(addr: SocketAddr) {
    let response = http_client::request(addr, "POST", &format!("/sessions/{CHAT_ID}/undo"), Some(r#"{"granularity":"turn"}"#));
...
async fn one_turn(upstream: &FakeServer, addr: SocketAddr) -> Vec<Value> {
    ...
    let input = http_client::request(addr, "POST", &format!("/sessions/{CHAT_ID}/input"), Some(r#"{"text":"你好"}"#));
    assert_eq!(input.status, 202, "{}", input.body);
    wait_for_terminal(&mut sse);

    let body = upstream.bodies().swap_remove(before);
    let body: Value = serde_json::from_str(&body).unwrap_or_else(|e| panic!("请求体不是 JSON：{e}\n{body}"));
