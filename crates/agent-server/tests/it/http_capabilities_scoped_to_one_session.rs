...
fn declaration_shuffled() -> Value {
    let Value::Object(mut map) = declaration() else {
        unreachable!()
    };
...
async fn a_declaration_only_reaches_the_session_that_declared_it() {
    ...
    create(
        server.addr,
        json!({ "id": "shuffled", "capabilities": declaration_shuffled() }),
    );
    ...
    assert!(
        !names(&baseline)
            .iter()
            .any(|n| n.starts_with("web:") || n.starts_with("desk:")),
        "不带声明的会话的表里不该出现任何注入的工具：{baseline:#?}"
    );

    // ── 注入排在表尾：前 N 项与基线**逐项相同**（不只是名字，整个 JSON 对象）。
    assert_eq!(
        &declared[..baseline.len()],
        &baseline[..],
        "所有会话共有的那一段一个字节都不许动（红线 11）"
    );
    assert_eq!(
        names(&declared)[baseline.len()..],
    ...
    assert_eq!(
        ...
        json!("按客户 ID 查 CRM 档案")
    );
    assert_eq!(
        lookup["function"]["parameters"]["properties"]["id"]["type"],
        json!("string")
    );
...
async fn tools_sent_by(upstream: &FakeServer, addr: SocketAddr, id: &str) -> Vec<Value> {
    ...
    let body: Value = serde_json::from_str(&body)
        .unwrap_or_else(|e| panic!("{id} 的请求体不是 JSON：{e}\n{body}"));
