...
async fn close_all_closes_every_session_created_over_http() {
    ...
    assert!(
        !id_a.is_empty() && !id_b.is_empty(),
        "两个 id 都该非空：{id_a:?} {id_b:?}"
    );

    let alive_before = sessions
        .ids()
        .iter()
        .map(|id| id.to_string())
        .collect::<Vec<_>>();
    assert!(
        alive_before.contains(&id_a) && alive_before.contains(&id_b),
        "两个新建的 session 该出现在 ids() 里：{alive_before:?}"
    );

    let outcomes = sessions.close_all();
    assert!(
        outcomes.iter().all(|(_, r)| r.is_ok()),
        "两个都是干净的活会话，close 不该报错：{outcomes:?}"
    );
    ...
    for id in [&id_a, &id_b] {
        ...
        assert_eq!(
            status.status, 404,
            "close 之后再查该是 404 not-found，body={}",
            status.body
        );
    }
}
