...
async fn repeated_chatid_reattaches_to_the_live_session_without_clearing_history() {
    ...
    assert_eq!(
        support::extract_json_string_field(&first.body, "id"),
        CHAT_ID
    );
    assert_eq!(
        support::extract_json_string_field(&first.body, "outcome"),
        "created"
    );
    ...
    assert_eq!(
        support::extract_json_string_field(&repeated.body, "id"),
        CHAT_ID
    );
    assert_eq!(
        support::extract_json_string_field(&repeated.body, "outcome"),
        "existing"
    );
    ...
    assert_eq!(
        request_bodies.len(),
        2,
        "两轮都该到达上游：{request_bodies:?}"
    );
    assert!(
        request_bodies[1].contains("remember-on-live-session"),
        "重复 POST 不能清空已有历史：{}",
        request_bodies[1]
    );
...
async fn closed_chatid_recovers_history_from_its_default_session_file() {
    ...
    let (_, _, mut first_sse) =
        http_client::connect_sse(first_addr, &format!("/sessions/{CHAT_ID}/events"), None);
    ...
    assert!(
        sessions_dir.join(format!("{CHAT_ID}.jsonl")).is_file(),
        "优雅关闭后恢复源文件该已完整落盘"
    );
    ...
    let (_, _, mut recovered_sse) =
        http_client::connect_sse(second_addr, &format!("/sessions/{CHAT_ID}/events"), None);
    ...
    assert_eq!(
        request_bodies.len(),
        2,
        "恢复后第二轮也该调用上游：{request_bodies:?}"
    );
    assert!(
        request_bodies[1].contains("remember-after-restart"),
        "恢复出的会话该携带关闭前的内容：{}",
        request_bodies[1]
    );
...
async fn omitting_chatid_keeps_the_legacy_generated_id_and_created_status() {
    ...
    assert!(
        support::extract_json_string_field(&response.body, "id").starts_with("sess-"),
        "{}",
        response.body
    );
    assert!(
        !response.body.contains("\"outcome\""),
        "旧请求的响应形状保持只含 id：{}",
        response.body
    );
...
fn post_input(addr: SocketAddr, id: &str, text: &str) {
    ...
    let response =
        http_client::request(addr, "POST", &format!("/sessions/{id}/input"), Some(&body));
...
fn wait_for_terminal(sse: &mut SseReader) {
    ...
    while std::time::Instant::now() < deadline {
        ...
        let frame: Frame = serde_json::from_str(&event.data)
            .unwrap_or_else(|error| panic!("SSE 帧不是 Frame：{error}: {}", event.data));
