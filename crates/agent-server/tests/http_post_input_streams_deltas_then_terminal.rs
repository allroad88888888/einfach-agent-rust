...
async fn posting_input_then_reading_sse_yields_deltas_then_a_terminal_frame() {
    ...
    let (sse_status, headers, mut sse) = http_client::connect_sse(server.addr, &format!("/sessions/{id}/events"), None);
    assert_eq!(sse_status, 200);
    assert!(
        headers.iter().any(|(k, v)| k.eq_ignore_ascii_case("cache-control") && v.eq_ignore_ascii_case("no-cache")),
        "{headers:?}"
    );
    ...
    while std::time::Instant::now() < deadline && terminal.is_none() {
        let Some(frame) = sse.next_event(Duration::from_secs(5)) else { break };
        assert!(frame.id.is_some(), "服务端每一帧都该带 id：{frame:?}");
        // 034：SSE 帧 data 是 `Frame` 信封，不再是裸的 `SessionEvent`。
        let envelope: Frame = serde_json::from_str(&frame.data).unwrap_or_else(|e| panic!("反序列化失败：{e}：{}", frame.data));
        match envelope.event {
            SessionEvent::TextDelta(delta) => text.push_str(&delta),
            SessionEvent::Notice(Notice::TurnStatusChanged { status }) if status.is_terminal() => terminal = Some(status),
            _ => {}
        }
    }

    assert_eq!(text, "hello from the model");
    assert!(matches!(terminal, Some(TurnStatus::Done { .. })), "该以正常结束的轮终态收尾：{terminal:?}");
}
