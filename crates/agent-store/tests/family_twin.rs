...
fn get_or_create_caches_per_key() {
    ...
    let a1 = family.get_or_create((0, 0), || {
        c.set(c.get() + 1);
        store.create_atom(num(1.0))
    });
    let c = creations.clone();
    let a2 = family.get_or_create((0, 0), || {
        c.set(c.get() + 1);
        store.create_atom(num(2.0))
    });
...
fn lazy_creation_only_on_use() {
    ...
    let id = family.get_or_create(7, || store.create_atom(num(7.0)));
...
fn evict_destroys_and_returns_store_to_baseline() {
    ...
    let mut family: AtomFamily<u32> = AtomFamily::new();
    for i in 0..1000 {
        family.get_or_create(i, || store.create_atom(num(i as f64)));
    }
...
fn evict_refuses_while_dependents_exist() {
    let store = Store::new();
    let mut family: AtomFamily<u32> = AtomFamily::new();
    let base = family.get_or_create(1, || store.create_atom(num(5.0)));
...
fn evict_refuses_while_subscribed() {
    let store = Store::new();
    let mut family: AtomFamily<u32> = AtomFamily::new();
    let id = family.get_or_create(1, || store.create_atom(num(0.0)));
...
fn detach_rekeys_without_destroying() {
    let store = Store::new();
    let mut family: AtomFamily<u32> = AtomFamily::new();
    let id = family.get_or_create(1, || store.create_atom(num(42.0)));
...
fn family_of_derived_atoms() {
    ...
    for i in 0..10 {
        cells.get_or_create(i, || store.create_atom(num(i as f64)));
    }
    let store_for_read = store.clone();
    let cell_5 = cells.get(&5).unwrap();
    let f = formulas.get_or_create(5, || {
        store_for_read
            .create_derived_ctx(move |args| num(args.get(cell_5).as_number().unwrap_or(0.0) * 10.0))
    });

    assert_eq!(store.get(f).as_number(), Some(50.0));
    store.set(cell_5, num(7.0));
