...
| 收紧子 agent 的成本兜底 | `--max-agent-depth <n>` / `--max-children <n>`（决策 32，默认 3/8）。**网关本身不需要改**——不传就是今天的行为；要非默认值就往上面那张参数表里加这两个。取值非法**拒绝启动**，不会静默退回默认档 |
| 管住「没人看着时自己跑」 | `--max-auto-turns <n>`（决策 35 / M20，默认 3，**0 = 整个关掉**）。M20 起会话在一轮结束后**可能自己再开一轮**（模型给 root 留了一条 `srv:agent/send` 的 `when="next_turn"`），这是本仓第一次在没有用户输入时继续消耗 token。只有**真实用户输入**能把这个预算加满，`begin_turn` 一个字都不碰；崩溃恢复出来是 0（恢复不自开）。企业侧要完全不要这个行为就传 `0` |

> **三道闸量的不是同一件事，估账要相乘。** `--max-agent-depth`/`--max-children` 量
> 「树有多大」，会话自己的 `MaxTurns` 量「一轮里说几次话」，`--max-auto-turns` 量
> 「没人看着时能跑几轮」。**用户按一次回车之后最坏花多少钱 = 这三样相乘**，
> 而不是其中任何一个。把它们当成同一道闸的三个名字，估出来的账会小一到两个数量级。
