...
**这条红线在 undo 还原钩子路径上的落点**（M19，决策 34）：还原函数不是新红线，是这条
规则在一个新阶段上的应用。`undo_turn`/`undo_step` 收一个调用方递进来的还原钩子，
在 `apply_prev`（把值写回状态）**之前**逐条调用——但顺序里还有一步更早：**钩子必须
跑在 bump 世代之后**。理由跟本条红线的理由是同一个：世代没 bump 之前跑钩子，跑的
这段时间里一条在飞的 tool 回执如果刚好落地，它带的还是旧世代号，会被当成合法结果
直接写进一个正在被撤销的世界。
