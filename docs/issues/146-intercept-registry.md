...
**里程碑** M16 · **依赖** — · **模型** sonnet · **独测** ✅ · **状态** 完成（2026-08-12）
...
- 落点：`intercept_registry.rs`（后经 147 按职责拆出 `session_tool_ext.rs` 公开层）。
  自借用解法：表 `Arc` 包住、建完不变，派发前 `Arc::clone` 断开借用。
- 实现 agent 的正确越权：注册工具缺省按名字规则落 `Irreversible`，故给公开层
  适配器补了 027 快照/屏障登记/ToolExecuting——不补是红线 6 静默洞。顺带
  `reply.rs` 的 `&'static str` 放宽为 `&str`（动态名所需，纯放宽零调用点改动）。
- 独测 6 条盲写全过：闭包真读 agent_tree、经 command 写出带 label 的 entry、
  Err 路、unknown_tool 路、未声明名注册 debug panic、未注册时首轮字节不变。
