...
§1 的预言兑现：`build.rs` 的 `let DerivedKey::ToolsConverged(agent) = key;` 加第二种
derived 时当场报 `refutable pattern in local binding`，改成 `match`。**这是好事**——
编译器逼下一个人在那儿回答「你这个 derived 读了什么」，而那正是新红线 10 唯一还有
意义的那半条判据的落点。

### 三态，不是布尔

`AwaitProgress::{Reached, Unreachable, Waiting}`。中间那一档规格里没有明说，但
不加就会有一类死等：`until = done` 而目标 `failed` 收场时，两态的实现只能一直
`Pending`，而泵会安静地返回。目标**不活着**那条同理，判在 `await_progress`
**之前**——一个死掉的 agent 的 `Status` 被 teardown 写回默认值 `Idle`，而 `Idle`
不是终态，拿它去问「到了没有」会答 `Waiting`，跟「还没开始跑」完全分不清。

### 独立测试 agent 逮到的：**被 await 盯着的后台子拆不掉**

它写「等的对象死了」那条验收时发现那条路根本走不到：`despawn` 的读者预检只把
`ToolsConverged` 当自己人，而 `AwaitReached` 的键里带着目标——于是运行时每转一圈
读一次进度就建起一条边，轮末 `orphan::reap` 恒回 `Kept`，那个子以活着的状态跨过
这一轮、又再也没有人驱动它，**等待方的槽从此永远 `Pending`**。

**查环挡得住「A 等 B、B 等 A」，挡不住「A 等一个已经没人驱动的 B」。** 修法见
`despawn.rs::own_derived_keys`（上一条 commit）。

它另外报了一条**诚实的空结果**：派活单建议的注入之一（把 `Slot::AwaitingOn` 的
默认值改成 `Null`）**一条测试都没红**——因为 `awaiting::from_value` 对 `Null` 和
`[]` 一视同仁（都解成空图，那是「形状不对整份读空」那条防御的副作用），而默认值
只在建 atom 时读一次、真正写过之后就不再经过它。它自己另找了一条有效的
（`to_value` 恒编码空数组），**两个 runtime 层的查环用例都红了**——那是「查环真的
在跑、不是空转成真」的直接证据。

### 真机（213 §1b）

正向那半过了：`root/a2` 用 `await` 等**兄弟** `root/a1`，真的挂住、A 干完之后收敛。
反向（互等被拒）真机上摆不出来——要摆出环得让 root 在同一轮里连着做两步，而模型
三次里两次把两步拆成两轮。环那条由上面的确定性夹具钉住，理由记在 213。
