...
## 追记（2026-08-13，issue 164）

§4 的 **60 秒默认值已被推翻**：`agent-wasm` 的 `HOST_TOOL_TIMEOUT` 改为
runtime 默认（10 分钟）。当年 60s 的前提是「这一头是个 JS 回调——机器干活」，
M14 之后页面工具包含人工参与的形状（提问 / 上传 / 确认），前提不再普遍成立；
两个宿主不再各自约定。理由与钉值测试见
[164](164-wasm-skills-declaration.md) §「123 默认截止线的变更」。
本文件正文保留原论证不改——那是当时成立的账。
