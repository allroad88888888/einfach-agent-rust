...
### 待办（两条 2026-08-13 全部收掉）

- [x] ~~GIF~~ —— [172](172-demo-gif.md) 录完，`docs/assets/undo-demo.gif`
      （760×500 / 77KB / 13s / 三帧带字幕），中英两份 README 都插在定位句与 demo 链接之后
- [x] ~~178 跑完之后把 quickstart 里 OpenAI 兼容那句改成「已实跑验证」~~ ——
      [178](178-openai-compat-dogfood.md) 八条真机全过，两份 README 的 quickstart 已改成：
      通用 adapter **在 DeepSeek / Kimi / GLM 三家上都真机跑过**，
      并列出跑过的面（工具调用、流式、缓存计数、鉴权失败、undo）。

      **措辞上克制了一处**：写的是「在上面三家上跑过」，不是「兼容任意 OpenAI 端点」。
      178 证明的是通用 adapter 在这三个端点上工作，推广到「任意兼容端点」需要的是
      别人的端点，不是我的断言。列出跑过哪几个面，比一个形容词有用——
      读者能自己判断他关心的那个面验没验过。
