...
**决策 16**：「`Encoded`（原 `ProviderRequest`）存在的理由是线程边界，不是组装」——
`store` 是 `Rc<RefCell>` 不 `Send`，HTTP 在别的线程，所以必须有一份「在 actor 线程上
提取、能带走」的东西。

**wasm 上没有那条线程边界。** 决策 16 的**理由**在这个目标下不成立了。
但 `Encoded` 作为「已提取、可带走的料」本身仍然有用（它同时是 `check_drift`
...
4. ✅ **决策 16 的表态 —— 已定：结论保留，理由改成双理由**。
   `Encoded` 在 native 上的理由是线程边界（store 是 `Rc<RefCell>` 不 `Send`）；
...
## 2026-08-11 补注：同步壳与「不做 `#[cfg]` 双份泵」的关系

本文拍板时写的是「一套路径、两边都 async，**不做 `#[cfg]` 双份泵**」，当时担心的具体
后果写在方案 A 的代价里：「`run_turn` 变 async 会一路传染到 `agent-server` / `agent-cli`」。

合并 M13 到 main 时加了一层**同步壳**：`run_turn_async` 是唯一的实现，
`run_turn` 是它的同步外壳（`block_on`，`#[cfg(not(target_arch = "wasm32"))]`）。

**这不是「双份泵」**：泵本体、IO 载体、落地规则两个目标共用同一份代码，
`#[cfg]` 只长在公开 API 的便利壳上。所以本文的决策没有被推翻。

**而且它把本文预计的那项代价直接消掉了**——传染没有发生，`agent-cli`、`agent-server`
和全部既有测试一行未改。本文 L86 / L115 / L147 里「agent-cli 需要 block_on」
之类的描述按此作废。
