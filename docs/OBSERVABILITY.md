...
宿主读整棵树 = 往下读每个 agent 的 `status` / `result` / `usage`——正是红线 10 sanctioned
的 `read_descendant` 方向。而且宿主**持有 store**，读的是自己拥有的状态，连横读都不算。

agent 之间**不**经这个接口互看——那还是红线 10 管的横读禁令。**可观测性是宿主 / UI 的
视角，不是 agent 的视角**：是「外面的人看这棵树」，不是「树里的节点互相看」。这条边界
定错了，会把「读所有兄弟」这种 O(n) 汇聚经可观测性的后门混进 agent 逻辑里。
