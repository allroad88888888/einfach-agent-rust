...
| **延迟加载**（能力多了不撑爆 prompt） | ✅ **完整且测过**。skill 的既有形状（写这份文档时是「常驻索引 + 模型激活才注入正文/工具」；141 换成「常驻索引 + 模型按需 `srv:skill/read` 取正文，不再携带工具」，结论不变——见 §八之二/`docs/TOOLS.md` §Skills）：**常驻索引**每个 skill 一行「名字 + 描述」，前缀稳定、近零成本。 |
...
| `existing`（会话还活在 registry 里） | 已装好、正在用 | 不要；**代码实况（158 真机核实，2026-08-12）：活会话带声明一律静默忽略**（sessions.rs 的理由：活会话换表 = 前缀缓存当场全断），本行原文「磁盘上有会话文件的一律拒绝」与代码分歧，待拍——见 ROADMAP §四 |
...
## 五、可逆性：声明只是自我描述，**宿主工具一律挡 undo**

> **`reversibility` 声明什么，都不再影响 `/undo` 停不停下来。**（决策 34 / issue
> [202](issues/202-host-mcp-undo-none.md)，2026-08-17）上面 §四 示例里那个
> `"reversibility": "pure"` 仍然合法、仍然会被打印出来，但它是一句**描述**，
> 不是一道**开关**。

### 为什么收窄

M10 这一节原话是「宿主是企业自己的代码，它说 `pure` 就按 `pure` 办——这是它自己的
数据，它自己负责」。M19 把判据收窄成一句话：**说了不算，交了才算，而宿主交不了。**

撤销一件外部世界里发生过的事，要跑的是**那个工具自己交回来的还原函数**
（决策 199 §一：`Γ → Γ × (Γ → Γ)`，执行的时候连同自己的逆一起返回）。宿主工具的
执行体在浏览器/桌面进程里，本仓手上只有一条 `POST /tool_result` 的回传通道，
**那条通道里没有「怎么撤」这半句话**。所以：

| 声明 | M19 之前 | 现在 |
|---|---|---|
| `irreversible`（含不声明） | 挡 undo | 挡 undo（**不变**） |
| `pure` | 不挡 | **挡** |
| `reversible` | 不挡 | **挡** |

后两行是一次**面向宿主的行为变更**，即便协议字段一个都没动。

`reversible` 那一行正是这次收窄的起点：宿主声明 `web:crm/draft` 是 `reversible`，
模型调它在 CRM 里建了一份草稿，用户 `/undo` —— 本仓打印「回退了 3 条目」，**没有
任何提示，而草稿还在 CRM 里**。「有补偿动作」这句话被当成了「有人会执行补偿动作」，
可从来没有人执行它。停下来问一次，比替用户假装撤销过要诚实。

### 用户仍然退得掉

`/undo` 撞上宿主工具会返回 `Blocked { cause: NoHook }`（「它没有提供还原函数，
本仓无从代它回退」），用户确认后 `/undo!`（`undo_turn_force`）**一次越过一条**
接着退。这是既有的屏障语义，没有新机制。

### 代价与没做的那半边

代价是真实的：一个只读页面标题的 `web:` 工具也会让那一轮 `/undo` 多问一次。
接受它，是因为反过来那个错误——**一次真的改了外部世界的调用被静默放过**——
不可逆，而且看起来完全正常。两个错误的代价不对称（这条判据本身没变，只是落点
从「填哪个枚举」变成了「交不交函数」）。

**宿主侧还原回调是第二步，本里程碑不做**：它要往宿主推「请还原」事件、等回传、
把 `undo_turn()` 从同步变成异步、还要定义还原失败怎么收场。等有真实宿主要它再开
issue（决策 199 §七）。

### 落地形状

照 `mcp_reversibility` 的既有先例，`ToolTable` 挂一张
`BTreeMap<Arc<str>, Reversibility>`，**不动 `ToolSpec` 的三字段形状**（它进 prompt，
加字段要重新算红线 11 的账；而可逆性纯查表、不进 prompt）。这张表现在只喂显示，
不喂行为——`dispatch` 的远端第五路无条件 `Session::mark_irreversible`，
不查它。显示层（CLI 的工具行、web 的工具卡片）打印这个字段时会带上
「本仓不代为补偿」，判据是 `agent_runtime::repo_cannot_compensate`。

**协议面不变**：`capabilities.tools[].reversibility` 仍然收 `pure`/`reversible`/
`irreversible` 三个值，**不会因为 `reversible` 不再有效果就变成 400**——拒绝一个
合法字段会让今天能建的会话建不起来，而它并没有变得更不安全（现在一律挡，比之前
更保守）。
...
（§一、§四示例、§十图）描述的是 139 之前的形状，激活子系统随
[141](issues/141-remove-activation-subsystem.md)已经正式删除（写入命令、执行授权、
料单正文段/中途工具段的展开逻辑都没了，`Slot::SkillsActive` 留壳只读）。
...
**校验四条**（400 且点名）：name 必须 `web:`/`desk:` 前缀（与内置 `srv:` 结构性
不撞名——这就是 ROADMAP §四欠的撞名判据）；前缀之后的本体过**与工具名一字不差的
白名单**（非空、`[A-Za-z0-9_/-]`、全名 ≤128——156 独测抓出初版漏了这层，随即
收紧：这个名字要进 journaled 的 `init:<name>` label、模型要在 `inherit_prefix`
里逐字打它）；声明内部、以及与 `capabilities.tools` 之间重名拒；`text` 为空拒
（声明常量空文本只能是笔误，069 判据）。长度上限不加，与 skill `body` 同一笔账
（§九 待拍安全项）。
