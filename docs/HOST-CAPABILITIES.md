  ...
  "capabilities": {
    "tools": [
      { "name": "web:crm/lookup",
        ...
        "reversibility": "pure" }          // 可选，只作显示，不决定 undo 挡不挡，见 §五
    ],
    "skills": [
      { "id": "crm-flow",
...
## 五、可逆性：声明只是自我描述，**承诺挡、事实不挡**

> **`reversibility` 这个字段本身不是开关——它描述的是事实还是承诺，才决定
> `/undo` 停不停下来。**（决策 34 / issue [202](issues/202-host-mcp-undo-none.md)，
> 2026-08-17，2026-08-18 review 修正）上面 §四 示例里那个 `"reversibility": "pure"`
> 仍然合法、仍然会被打印出来，且**确实不挡**——它描述的是「没碰外部世界」这个事实；
> `"reversible"` 同样合法、同样会被打印，但**挡**——它描述的是这条通道交不出来的
> 一个承诺。

### 为什么收窄

M10 这一节原话是「宿主是企业自己的代码，它说 `pure` 就按 `pure` 办——这是它自己的
数据，它自己负责」。M19 把判据收窄成一句话：**说了不算，交了才算**——但「交」这件事
只对**承诺**成立，对**事实**不成立：一件没碰过外部世界的事，不需要交任何东西来证明
它没发生过。

撤销一件外部世界里发生过的事，要跑的是**那个工具自己交回来的还原函数**
（决策 199 §一：`Γ → Γ × (Γ → Γ)`，执行的时候连同自己的逆一起返回）。宿主工具的
执行体在浏览器/桌面进程里，本仓手上只有一条 `POST /tool_result` 的回传通道，
**那条通道里没有「怎么撤」这半句话**——但这句话只挡得住**承诺**（`reversible`
声明的「有补偿动作」），挡不住**事实**（`pure` 声明的「没碰外部世界」）。所以：

| 声明 | M19 之前 | 现在 |
|---|---|---|
| `irreversible`（含不声明） | 挡 undo | 挡 undo（**不变**） |
| `pure` | 不挡 | 不挡（**不变**——它是事实，不是交不出的承诺） |
| `reversible` | 不挡 | **挡**（唯一变化） |

**只有 `reversible` 那一行是一次面向宿主的行为变更**，即便协议字段一个都没动。
初稿曾经写成「声明什么都挡」，落地时发现连 `ask_user_question` / `browser_action`
这类字面上不可能有副作用的标准集工具也会被一起挡住——挡住它们保护不了任何东西，
只会让「模型问了一句话」那一轮撤不掉，review 时（2026-08-18）改回了现在这版
只挡承诺、放行事实的判据。
...
代价是真实的，但收窄到只挡承诺之后小了一档：只有声明 `reversible` 的宿主工具才会
让那一轮 `/undo` 多问一次——声明 `pure` 的工具（比如一个只读页面标题的 `web:` 工具）
不受影响。接受剩下这份代价，是因为反过来那个错误——**一次真的改了外部世界的调用
被静默放过**——不可逆，而且看起来完全正常。两个错误的代价不对称（这条判据本身没变，
只是落点从「填哪个枚举」变成了「交不交函数」）。
...
不喂行为——`dispatch` 的远端第四/第五路按 `agent_runtime::is_unkeepable_promise(&request)`
有条件地 `Session::mark_no_undo`（`pure` 声明的事实断言判 `false`、不落这一位；
只有落不到函数的 `reversible` 承诺才落），不查这张显示表。显示层（CLI 的工具行、
web 的工具卡片）打印 `reversible` 时会带上「本仓不代为补偿」，判据同一个
`is_unkeepable_promise`。

**协议面不变**：`capabilities.tools[].reversibility` 仍然收 `pure`/`reversible`/
`irreversible` 三个值，**不会因为 `reversible` 不再有效果就变成 400**——拒绝一个
合法字段会让今天能建的会话建不起来，而它并没有变得更不安全（新增挡的只有
`reversible` 这一档，`pure` 待遇不变，整体比之前更保守）。
