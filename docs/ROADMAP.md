...
| 34 | **可逆性从声明的枚举改成交付的还原函数**（2026-08-17，[199](issues/199-reversibility-as-delivery-decision.md)）：工具执行完顺手交回一个**三态** `Aftermath`（`Nothing` 没碰外部世界 / `Undo(f)` 碰了且给出还原函数 / `Irreversible` 碰了还不回去 = 撞上屏障）；可逆性因此是**每次调用**的属性、不是每个工具的属性。**不用 `Option<UndoFn>`**——那会把「没碰」和「碰了但撤不回」压成同一个 `None`，而落盘那一位是三态，返回类型必须与它同构，否则装配那一步只能猜（初稿写成两态，review 时改的）。还原函数**住 runtime 不住 core**（红线 7）：core 只记账三态 `Undoability`（`StateOnly` 没碰外部世界 / `Hooked` 碰了且交了函数 / `Blocked` 碰了没交），钩子表按 `Entry::seq` 存在 runtime。**顺序钉死**：`undo_turn`/`undo_step` 在 `apply_prev`（store 回滚）**之前**逐条调用还原函数，顺序反了是「store 说没发生、外部说发生了」的静默错值，本仓最容易埋的那类洞（红线导言）。**还原函数是闭包，不跨进程**：崩溃恢复后钩子表是空的，`Hooked` 却查不到 → 按还原失败处理而不是静默当 `StateOnly`。**失败复用既有屏障**（`UndoReport::Blocked` 加成因字段 + `undo_turn_force()` 逐条越过）——**否决** `ignore_undo_errors: bool` 一类开关，那是「事先设一次替用户答了所有还没被问到的问题」，`undo.rs` 已经否决过同形状。`srv:agent/spawn` 交 `Nothing`→`StateOnly`（它在**外部世界**留下的是零，子树回滚由同一条日志承担；初稿写「交 no-op」是 `Option` 时期的遗留，判 `Hooked` 反而会让恢复后查不到钩子 → `HookLost` → 每次 spawn 都变屏障）；执行体在别的进程里的两类（宿主 `web:`/`desk:`、MCP）交不回函数，判据是**声明的是事实还是承诺**：`pure`/`readOnlyHint:true` 声明的是「没碰外部世界」这个**事实**，不需要函数来兑现，照采信落 `StateOnly`（决策 22 因此不被反转，`ask_user_question` 这类也不会被无故挡住）；`reversible` 声明的是「有补偿动作」这个**承诺**，而承诺要兑现就得交出那个结构上交不出来的函数，落 `Blocked`——**这一格正是本决策存在的理由**；其余一律 `Blocked`。声明本身降级为纯显示标签 | 用户读论文（Cordis，PKU+DeepSeek-AI，《A Programming Paradigm for Spatiotemporal Composability》）后点破的根上错误：`Reversibility` 是「我们在给一件看不见的事做分类」，依据只有工具名和宿主自填的字符串——**标签可以吹，函数不给就是没有**。清账证实今天 `Reversible` 唯一的差别是打印给人看的字符串，边界外（宿主声明）没有任何人执行补偿。**明确不做三条**（记在这里防止以后重开讨论）：不让模型/子 agent「想办法撤销」——函数失败会报错会停下，模型「撤销」会说一句「已清理完毕」然后什么都没做，而你没有任何办法知道；**宿主侧还原回调是第二步**，等真实宿主要它再开（见 §四）；**论文的空间维**（reactive coeffects：依赖声明+运行期重新连线）不做——我们的工具表由宿主一次性装配、会话开始时交给模型、运行实例内不再变，论文那半边解决的问题这里从源头不存在，且运行期重连正是红线 11 的正对面（038 探针：中途改工具数组在 DeepSeek 上把缓存归零，120 倍的钱）。落地见 200–203 |
| 35 | **agent 之间互相看得见、说得上话；红线 10 从「方向约束」改成「边只许指向 primitive」**（2026-08-18，[204](issues/204-agent-mesh-decision.md)，用户拍板「红线 10 干掉——我的意思是可以横着读」「1 可以互相订阅 2 自己开一轮」）：**横读全开，含响应式订阅**。地基是一次勘查而不是一次权衡——`graph/build.rs:71` 的 `let DerivedKey::ToolsConverged(agent) = key;` 说明**整个系统只有一种 derived**，`Slot` 里每一个都是 primitive，且这是**类型上的事实**（source 与 derived 是两张按不同键类型索引的 family，`build.rs:47`/`:53` 的注释：「让『快照只存 primitive』成为类型上的事实」）。跨 agent 读读的是 `AtomKey::Agent(id, slot)`，**永远落在 source family 上、永远是叶子**；而环需要 derived→derived 绕回来——**所以横向订阅造不出环，跟方向毫无关系**，`U ∩ D = ∅` 那个证明防的是一件在当前实现下已经不可能的事。新判据（**跨 agent 的边只许指向 primitive**）比旧的更简单也更强，且今天**由类型系统守着**。`Visibility` 三态收成两态（`Shared`/`Private`），**穷举 match 保留**——它是红线 10 唯一能被编译器守住的部分，跟方向无关，不能跟着方向一起删；`read_ancestor`/`read_descendant` **保留成带方向断言的薄封装**（现有调用方与测试一行不改，照 200 保留无参 `undo_turn()` 的同一条理由：一次全改是无谓的爆炸半径），新增**一个**口 `read_agent`（不查方向、只查 `Private`）——**不是两个**：开工核查发现 `read_ancestor`/`read_descendant` 走 `Session::peek` → `store.get`（`tree.rs:126`）是**命令层的非追踪读，本来就不建边**，`Session` 层根本没有「建边的读」这回事，两个名字会是同一个实现。**响应式那条边第一次出现是在 212 的 `await` derived 里**（`args.get` 在生产代码里此前只有 `build.rs:103` 一处、读的还是自己 agent 的槽位——**全系统在此之前没有一条跨 agent 的依赖边**），所以新判据的落地与测试都在 212，不在 205。**否决整条删掉 `Visibility`**：买到的正好是零（要的四件事没一件需要「derived 依赖兄弟的槽位」），卖掉的是那个无通配的穷举 match、断言集合性质的测试、以及「环不可能」本身。**否决跨 agent 读 derived**——它是新判据现在唯一的依据。**代价一，照实记**：`Messages` 从 Upward-only 变 `Shared`，意味着谁都读得到谁的完整正文，一次就能把一轮成本翻几倍——**core 放行、工具层不给模型开这条路**（要正文有 `collect` 和 `send`，两条都有边界）。**代价二，也是真订阅唯一的新危险：死锁**。A `await` B、B `await` A，两边都不会动，而**泵会安静地返回**（静止条件 `calls.is_empty() && mcp_calls.is_empty()` 成立——两个互等的 agent 都没有 provider 调用在飞），不 panic、不超时、不告警，留下两个永远 `Pending` 的槽。挡法是一张 **journaled 的等待图**（`Slot::AwaitingOn`；放内存里，一次崩溃恢复就把查环能力丢了，而丢了不报错）+ **在建立那一刻查环并拒**；**否决「卡住之后检测死锁并挑一个牺牲者」**——建立时挡得住的事不该留到运行期用更复杂的机械去救。**消息两档送达时机**（用户拍的）：`now` 加入本轮 loop、收信人下一次组装请求之前排空，目标是本轮任意活 agent（含兄弟）；`next_turn` 这一轮结束之后送达、**目标只能是 root**——子 agent 不跨 turn（ORCHESTRATION §二/§四.4 的既有结论），投给别人等于投给一个下一轮不存在的收件箱，故**当场显式拒**而不是投完静默丢。**一个槽两个标记，不是两个槽**（落盘/恢复/undo/`Private` 四样逐字相同，拆开就要写两遍且必须永远一致）；`next_turn` 的排空点在 `begin_turn` **之后**——放之前那条留言会挂在上一轮尾巴上，undo 掉上一轮就把一条还没被读过的消息一起吞了。落点一句话：**消息跨 turn，agent 不跨 turn**，所以 ORCHESTRATION §六 延后的那套跨 turn 机械（pending-slot 跨 `run_turn` 重挂、`turn_id`/undo 重写、per-child 取消）**一样都不需要**；**否决跨 turn 复活已死的子 agent**——跨轮传递的是话不是 agent。**留言会自己把下一轮开起来**（用户拍的）：本仓第一次在**没有用户输入**时继续消耗 token，这把停机保证拆掉了（`TurnsUsed` 在 `begin_turn` 时重置为 0，`txn.rs:274`，而自开的每一轮都是真的新一轮，重置合法），故配一道**会话级 `Slot::AutoTurnBudget`**：**只有真实用户输入才加满**，`begin_turn` 一个字都不许碰它，见底就停下、留言原地留着。三条附带拍板——**恢复不自动开轮**（否则打开应用它自己就开始烧钱，而用户还没看到上一轮发生了什么）、**undo 不退还预算**（钱已经烧了；退还就交出一条「撤销→重跑→再撤销」的无限循环，且它看起来像 bug，注释要写死）、**三个面都要分得出「这轮是它自己开的」并能当场喊停**（浏览器里没有 Ctrl-C，那儿比 CLI 更要紧）。**否决预算按时间/按进展自动续期**——任何自动续期都是把闸接回被它约束的那个循环里。**「改自己状态」换形状不换写口**：现有槽位一格一格数过去**没有一格是模型自己的**（`MaxTurns`/`MaxRetries` 是部署方的、`ToolsAllowed` 是父给的、`SendPlan`/`Summaries` 是 adapter 的、`Status` 是别人要读的结论），开写口 = 让被约束者改自己的约束，是决策 32 往反方向走一遍；**否决给现有槽位开写口**，改为新增 `Slot::Notes`（`Private`、`BTreeMap`、`StateOnly`），它白拿 undo/崩溃恢复/审计。落地 M20（204–209、211–213） | ①**地基是查出来的，不是想出来的，而且查了两遍才对**：初稿把「互相订阅」定价成贵档（要放弃结构性无环保证、退化成运行时检测）；查 `build.rs:71` 后改成「几乎白拿」（`Slot` 全 primitive ⇒ 跨 agent 的边都是长度 1 的悬边）；**落地前照 WORKFLOW §四第 0 步再查一遍，发现还要更空**——红线 10 那两个口是命令层的非追踪读、**从不建边**，且**没有任何生产调用方**（四十处引用全在测试里），而 `build.rs:90` 那句「M3 的『等所有子 agent 完成』会在同一个位置汇聚」是**将来时**（029 的汇聚 derived 从没建过，运行时用 `Subtree::harvest` 命令式做了）。**所以红线 10 至此是一道架在没人调的 API 上、防着一类还不存在的边的策略闸，放开它的代价是零**。这条记在这里不是为了自嘲：它正是 WORKFLOW 第 0 步「别信任何文档对『已完成』的描述」的一次兑现，而这次被推翻的文档是一小时前自己写的。幸好那个无环证明仍然要紧：真撞上环时 `store/read.rs:151` 是 `panic!("circular dependency detected")`，**不是错误返回**——一个模型可触发的 panic 完全不能接受，所以判据必须在 212 建出第一条跨 agent 边的那一刻由断言接住 ②**三个陷阱都不报错**，这是本仓最贵的那类洞（红线导言）：唤醒若走 `begin_turn` 的重置路 → 两个 agent 互相喊话真无界；`begin_turn` 若碰 `AutoTurnBudget` → 闸接回被它约束的循环，等于没有闸；死锁 → 泵安静返回，没有任何人有能力发现它。三条各有一条专门的断言，都写着「别删」 ③停机论证从一层变三层，三道闸各管一件事不能兼任（树多大 / 一轮说多少话 / 没人看着时跑几轮），**三个数相乘就是一次用户输入之后的最坏开销**——这个数要写进启动参数文档，部署方靠它估账（它同时把 §四「单 session token 预算闸」那条的优先级顶了上来：三道闸给的都是轮数上界，而自驱动会话恰恰最需要花费上界）④决策 32 刚把两道闸从代码搬到部署方手里，同一波里再把 `MaxTurns` 塞进模型手里就是自相矛盾；给它一张自己的草稿纸则一个不变量都不碰，而且 undo/恢复/审计是这套架构直接掉出来的，不是新造的机制 |
...
- **要不要单 session 的 token 预算闸**（M1 用完真实数据再定）。`max_turns` 是**轮数**闸，
  ...
  **决策 35 之后这条的优先级上来了**：`AutoTurnBudget` 又是一道**轮数**闸，
  三道闸相乘（树多大 × 一轮说多少话 × 没人看着时跑几轮）给的仍然是轮数上界，
  而自驱动会话恰恰是「没人在旁边看着账单」的那种场景——真正想要的是花费上界。
  等 M20 真机跑出第一批自驱动会话的实际花费再定，别提前猜一个数。
...
- **宿主侧还原回调**（决策 34 §七/§十，M19 之后）：决策 34 只解决了「工具自己交
  还原函数」这一半——`web:`/`desk:` 的还原函数活在宿主那边，本仓今天交不回来，
  一律 `Blocked`（撞上屏障停下问）。第二步是往宿主推一个「请还原」事件、等它回传
  结果，这要动协议、要让 `undo_turn()` 从同步变异步、还要定义还原失败在跨进程往返
  之后怎么收场（超时？宿主进程已经不在了？）。等有真实宿主要它再开 issue，别提前猜。
